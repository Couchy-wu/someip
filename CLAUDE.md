...
- ⚠ **未插卡时底层 VCI 驱动会段错误**（实测 rc=139，try/except 拦不住）→ 任何"打开设备"之前
  先跑 `can_core.device_probe.probe_can_device()`（子进程探测，结果带 30s 缓存；按**模块属性**调用
  便于替换/测试）；已接入 CAN 测试界面的「检测设备」按钮、初始化前预检，以及 Di 窗口执行前预检；
...
> 已知取舍（回退布局后仍在）：SOME/IP 单行工具栏给状态栏的空间有限（已 `anchor="w"`
> 保证先看到"库不可用 + 原因"开头，完整提示在⑤日志区）。**默认窗口已按用户确认从
> 1120×780 加宽到 1280×780**（只改窗口大小，控件摆放未动）：实测 1120 宽时
> ④ 行「发送该事件」只剩 4px、状态栏被裁到 322px；1280 宽时为 119px / 368px，均放得下。
> 库不可用时的长文案（约 822px）仍建议拉宽窗口或看日志区。
