...
## 7.5 双机（跨主机）通信 —— 支持，需 3 处配置调整（已实测）

**架构上 vsomeip 天生支持双机**：每台主机各自有本地 RM（服务端自持 / 客户端首应用自持或 vsomeipd），
主机间通过 **SD 多播发现 + 网络 UDP/TCP 数据通道**通信，无需共享 UDS。

```
主机A(服务端)                    主机B(客户端)
┌────────────────┐             ┌─────────────────┐
│ 应用(arhud01)  │             │ 应用 arhud_client_0 = B机RM宿主 │
│ RM 宿主 ──offers──┐           │ RM 宿主(proxy→远端)          │
└────────┬───────┘ │           └────────┬────────┘
         │ SD多播 224.0.2.4:30490       │
         ◄──────────────────────────────►
         │ 网络UDP 51400-51409/52001    │
         ◄────────── notify ────────────►
```

与单机的区别（已用双容器/双网络命名空间实测，B 机收齐 A 机 23/23 事件）：

| 配置项 | 单机 | 双机 |
|---|---|---|
| `unicast` | 127.0.0.1 即可 | **各自主机真实 IP**（`get_local_ip` 自动探测含默认网关/无外网回退，或 `ARHUD_UNICAST` 指定） |
| 客户端 `routing` | 服务端第一个应用名（同机） | **必须指向本机 RM 宿主**！B 机无服务端时：`ARHUD_ROUTING_HOST=arhud_client_0`（客户端首应用自持 RM），或 B 机跑 vsomeipd。**不能指向 A 机的应用名**（本地通道连不到对端） |
| service-discovery | enable=true | enable=true（**多播 224.0.2.4:30490 必须在网络可达**） |
| 防火墙 | 一般无需 | 放行 UDP 51400-51409/52001、30490（含多播） |
| 启动顺序 | 先服务端后客户端 | 先 A 机服务端，再 B 机客户端 |

验证方式（仓库内自动化）：`docker/integration_test_dual_host.sh`（创建自定义 bridge 网络 →
A 容器服务端 / B 容器客户端 → 断言 B 机 `已收 23/23`），已纳入 `run_tests.sh` 与 CI。
