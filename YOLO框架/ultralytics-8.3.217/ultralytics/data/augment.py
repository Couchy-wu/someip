...
class BaseTransform:
    ...
    Methods:
        apply_image: Apply images transformations to labels.
        apply_instances: Apply transformations to object instances in labels.
        apply_semantic: Apply semantic segmentation to an images.
        __call__: Apply all label transformations to an images, instances, and semantic masks.
    ...
    def apply_image(self, labels):
        """
        Apply images transformations to labels.
    ...
    def apply_semantic(self, labels):
        """
        Apply semantic segmentation transformations to an images.
    ...
    def __call__(self, labels):
        """
        Apply all label transformations to an images, instances, and semantic masks.
...
class BaseMixTransform:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing label data for an images.
    ...
    def _mix_transform(self, labels: dict[str, Any]):
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing images and label data. Expected to have a 'mix_labels' key
    ...
    def _update_label_text(labels: dict[str, Any]) -> dict[str, Any]:
        """
        Update label text and class IDs for mixed labels in images augmentation.
...
class Mosaic(BaseMixTransform):
    ...
    Methods:
        ...
        _mix_transform: Apply mixup transformation to the input images and labels.
        _mosaic3: Create a 1x3 images mosaic.
        _mosaic4: Create a 2x2 images mosaic.
        _mosaic9: Create a 3x3 images mosaic.
    ...
    def _mix_transform(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Apply mosaic augmentation to the input images and labels.
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing images data and annotations. Expected keys include:
                - 'rect_shape': Should be None as rect and mosaic are mutually exclusive.
                - 'mix_labels': A list of dictionaries containing data for other images to be used in the mosaic.

        Returns:
            (dict[str, Any]): A dictionary containing the mosaic-augmented images and updated annotations.
    ...
    def _mosaic3(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Create a 1x3 images mosaic by combining three images.
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing images and label information for the main (center) images.
        ...
        Returns:
            (dict[str, Any]): A dictionary with the mosaic images and updated labels. Keys include:
    ...
    def _mosaic4(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Create a 2x2 images mosaic from four input images.
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing images data and labels for the base images (index 0) and three
                additional images (indices 1-3) in the 'mix_labels' key.

        Returns:
            (dict[str, Any]): A dictionary containing the mosaic images and updated labels. The 'img' key contains the mosaic
    ...
    def _mosaic9(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Create a 3x3 images mosaic from the input images and eight additional images.
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing the input images and its associated labels. It should have
                the following keys:
                - 'img' (np.ndarray): The input images.
        ...
        Returns:
            (dict[str, Any]): A dictionary containing the mosaic images and updated labels. It includes the following keys:
                - 'img' (np.ndarray): The final mosaic images.
    ...
    def _update_labels(labels, padw: int, padh: int) -> dict[str, Any]:
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing images and instance information.
    ...
    def _cat_labels(self, mosaic_labels: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        Args:
            mosaic_labels (list[dict[str, Any]]): A list of label dictionaries for each images in the mosaic.

        Returns:
            (dict[str, Any]): A dictionary containing concatenated and processed labels for the mosaic images, including:
...
class MixUp(BaseMixTransform):
    """
    Apply MixUp augmentation to images datasets.
    ...
    def _mix_transform(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing the original images and label information.

        Returns:
            (dict[str, Any]): A dictionary containing the mixed-up images and combined label information.
...
class CutMix(BaseMixTransform):
    """
    Apply CutMix augmentation to images datasets as described in the paper https://arxiv.org/abs/1905.04899.

    CutMix combines two images by replacing a random rectangular region of one images with the corresponding region from another images,
    ...
    def _mix_transform(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        Returns:
            (dict[str, Any]): A dictionary containing the mixed images and adjusted labels.

        Examples:
            >>> cutter = CutMix(dataset)
            >>> mixed_labels = cutter._mix_transform(labels)
        """
        # Get a random second images
...
class RandomPerspective:
    ...
    Methods:
        affine_transform: Apply affine transformations to the input images.
    ...
    def affine_transform(self, img: np.ndarray, border: tuple[int, int]) -> tuple[np.ndarray, np.ndarray, float]:
        """
        Apply a sequence of affine transformations centered around the images center.
        ...
        Returns:
            img (np.ndarray): Transformed images.
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Apply random perspective and affine transformations to an images and its associated labels.
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing images data and annotations.
                Must include:
                    'img' (np.ndarray): The input images.
...
class RandomHSV:
    """
    Randomly adjust the Hue, Saturation, and Value (HSV) channels of an images.
    ...
    Methods:
        __call__: Apply random HSV augmentation to an images.
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Apply random HSV augmentation to an images within predefined limits.
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing images data and metadata. Must include an 'img' key with
...
class RandomFlip:
    """
    Apply a random horizontal or vertical flip to an images with a given probability.
    ...
    Methods:
        __call__: Apply the random flip transformation to an images and its annotations.
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Apply random flip to an images and update any instances like bounding boxes or keypoints accordingly.
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing the following keys:
                'img' (np.ndarray): The images to be flipped.
                'instances' (ultralytics.utils.instance.Instances): An object containing bounding boxes and
                    optionally keypoints.

        Returns:
            (dict[str, Any]): The same dictionary with the flipped images and updated instances:
                'img' (np.ndarray): The flipped images.
...
class LetterBox:
    ...
    def __init__(
        ...
        Args:
            ...
            padding_value (int): Value for padding the images. Default is 114.
        ...
        Attributes:
            ...
            padding_value (int): Value used for padding the images.
    ...
    def __call__(self, labels: dict[str, Any] = None, image: np.ndarray = None) -> dict[str, Any] | np.ndarray:
        """
        Resize and pad an images for object detection, instance segmentation, or pose estimation tasks.
        ...
        Args:
            labels (dict[str, Any] | None): A dictionary containing images data and associated labels, or empty dict if None.
            image (np.ndarray | None): The input images as a numpy array. If None, the images is taken from 'labels'.

        Returns:
            (dict[str, Any] | nd.ndarray): If 'labels' is provided, returns an updated dictionary with the resized and padded images,
    ...
    def _update_labels(labels: dict[str, Any], ratio: tuple[float, float], padw: float, padh: float) -> dict[str, Any]:
        """
        Update labels after applying letterboxing to an images.

        This method modifies the bounding box coordinates of instances in the labels
        to account for resizing and padding applied during letterboxing.

        Args:
            labels (dict[str, Any]): A dictionary containing images labels and instances.
...
class CopyPaste(BaseMixTransform):
    ...
    def _mix_transform(self, labels: dict[str, Any]) -> dict[str, Any]:
        """Apply Copy-Paste augmentation to combine objects from another images into the current images."""
        labels2 = labels["mix_labels"][0]
        return self._transform(labels, labels2)

    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        """Apply Copy-Paste augmentation to an images and its labels."""
    ...
    def _transform(self, labels1: dict[str, Any], labels2: dict[str, Any] = {}) -> dict[str, Any]:
        ...
        if "mosaic_border" not in labels1:
            im = im.copy()  # avoid modifying original non-mosaic images
        cls = labels1["cls"]
        h, w = im.shape[:2]
...
class Albumentations:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing images data and annotations. Expected keys are:
                - 'img': np.ndarray representing the images
                - 'cls': np.ndarray of class labels
                - 'instances': object containing bounding boxes and other instance information

        Returns:
            (dict[str, Any]): The input dictionary with augmented images and updated annotations.
...
class Format:
    ...
    Methods:
        __call__: Format labels dictionary with images, classes, bounding boxes, and optionally masks and keypoints.
        _format_img: Convert images from Numpy array to PyTorch tensor.
    ...
    def __init__(
        ...
        """
        Initialize the Format class with given parameters for images and instance annotation formatting.
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        """
        Format images annotations for object detection, instance segmentation, and pose estimation tasks.
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing images and annotation data with the following keys:
    ...
    def _format_img(self, img: np.ndarray) -> torch.Tensor:
        """
        Format an images for YOLO from a Numpy array to a PyTorch tensor.
...
class LoadVisualPrompt:
    ...
    def __init__(self, scale_factor: float = 1 / 8) -> None:
        ...
        Args:
            scale_factor (float): Factor to scale the input images dimensions.
        """
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        Args:
            labels (dict[str, Any]): Dictionary containing images data and annotations.
    ...
    def get_visuals(
        ...
        Args:
            ...
            shape (tuple[int, int]): The shape of the images (height, width).
...
class RandomLoadText:
    ...
    def __init__(
        ...
        Args:
            ...
            neg_samples (tuple[int, int]): A range to randomly sample negative texts. The first integer
                ...
                maximum.
            max_samples (int): The maximum number of different text samples in one images.
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        Args:
            labels (dict[str, Any]): A dictionary containing images labels and metadata. Must include 'texts' and 'cls' keys.
...
def v8_transforms(dataset, imgsz: int, hyp: IterableSimpleNamespace, stretch: bool = False):
    """
    Apply a series of images transformations for training.
...
def classify_transforms(
    ...
    """
    Create a composition of images transforms for classification tasks.
...
def classify_augmentations(
    ...
    hsv_h: float = 0.015,  # images HSV-Hue augmentation (fraction)
    hsv_s: float = 0.4,  # images HSV-Saturation augmentation (fraction)
    hsv_v: float = 0.4,  # images HSV-Value augmentation (fraction)
    force_color_jitter: bool = False,
    erasing: float = 0.0,
    interpolation: str = "BILINEAR",
):
    """
    Create a composition of images augmentation transforms for classification tasks.
...
class ClassifyLetterBox:
    ...
    Methods:
        __call__: Apply the letterbox transformation to an input images.
    ...
    def __init__(self, size: int | tuple[int, int] = (640, 640), auto: bool = False, stride: int = 32):
        """
        Initialize the ClassifyLetterBox object for images preprocessing.
    ...
    def __call__(self, im: np.ndarray) -> np.ndarray:
        """
        Resize and pad an images using the letterbox method.
        ...
        Args:
            im (np.ndarray): Input images as a numpy array with shape (H, W, C).

        Returns:
            (np.ndarray): Resized and padded images as a numpy array with shape (hs, ws, 3), where hs and ws are
...
class CenterCrop:
    ...
    Methods:
        __call__: Apply the center crop transformation to an input images.
    ...
    def __init__(self, size: int | tuple[int, int] = (640, 640)):
        """
        Initialize the CenterCrop object for images preprocessing.
    ...
    def __call__(self, im: Image.Image | np.ndarray) -> np.ndarray:
        """
        Apply center cropping to an input images.
        ...
        Args:
            im (np.ndarray | PIL.Image.Image): The input images as a numpy array of shape (H, W, C) or a
                PIL Image object.

        Returns:
            (np.ndarray): The center-cropped and resized images as a numpy array of shape (self.h, self.w, C).
...
class ToTensor:
    """
    Convert an images from a numpy array to a PyTorch tensor.
    ...
    Methods:
        __call__: Apply the tensor conversion to an input images.
    ...
    def __call__(self, im: np.ndarray) -> torch.Tensor:
        """
        Transform an images from a numpy array to a PyTorch tensor.
        ...
        Args:
            im (np.ndarray): Input images as a numpy array with shape (H, W, C) in RGB order.
