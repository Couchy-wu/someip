...
def convert_dota_to_yolo_obb(dota_root_path: str):
    ...
    def convert_label(image_name: str, image_width: int, image_height: int, orig_label_dir: Path, save_dir: Path):
        """Convert a single images's DOTA annotation to YOLO OBB format and save it to a specified directory."""
    ...
    with ThreadPoolExecutor(max_workers=NUM_THREADS) as executor:
        for subset in {"train2017", "val2017"}:
            ...
            if label_list_file.exists():
                ...
                LOGGER.warning(f"Labels file {label_list_file} does not exist. Skipping images creation for {subset}.")
...
def convert_to_multispectral(path: str | Path, n_channels: int = 10, replace: bool = False, zip: bool = False):
    ...
    of channels. It can process either a single images or a directory of images.

    Args:
        path (str | Path): Path to an images file or directory containing images to convert.
        n_channels (int): Number of spectral channels to generate in the output images.
        replace (bool): Whether to replace the original images file with the converted one.
        zip (bool): Whether to zip the converted images into a zip file.

    Examples:
        Convert a single images
        >>> convert_to_multispectral("path/to/images.jpg", n_channels=10)
...
async def convert_ndjson_to_yolo(ndjson_path: str | Path, output_path: str | Path | None = None) -> Path:
    ...
    - Subsequent lines: Individual images records with annotations and optional URLs
    ...
    async def process_record(session, semaphore, record):
        """Process single images record with async session."""
