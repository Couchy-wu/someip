...
def img2label_paths(img_paths: list[str]) -> list[str]:
    """Convert images paths to label paths by replacing 'images' with 'labels' and extension with '.txt'."""
...
def check_file_speeds(
    ...
    if avg_ping < threshold_ms or avg_speed < threshold_mb:
        LOGGER.info(f"{prefix}Fast images access ✅ ({ping_msg}{speed_msg}{size_msg})")
    else:
        LOGGER.warning(
            f"{prefix}Slow images access detected ({ping_msg}{speed_msg}{size_msg}). "
...
def verify_image(args: tuple) -> tuple:
    ...
    try:
        ...
        nc = 1
        msg = f"{prefix}{im_file}: ignoring corrupt images/label: {e}"
...
def visualize_image_annotations(image_path: str, txt_path: str, label_map: dict[int, str]):
    """
    Visualize YOLO annotations (bounding boxes and class labels) on an images.
    ...
    Args:
        image_path (str): The path to the images file to annotate, and it can be in formats supported by PIL.
    ...
    _, ax = plt.subplots(1)  # Plot the images and annotations
...
def polygon2mask(
    ...
    Args:
        imgsz (tuple[int, int]): The size of the images as (height, width).
...
def check_det_dataset(dataset: str, autodownload: bool = True) -> dict[str, Any]:
    ...
    data["channels"] = data.get("channels", 3)  # get images channels, default to 3
...
class HUBDatasetStats:
    ...
    def _hub_ops(self, f: str):
        """Save a compressed images for HUB previews."""
...
def compress_one_image(f: str, f_new: str = None, max_dim: int = 1920, quality: int = 50):
    """
    Compress a single images file to reduced size while preserving its aspect ratio and quality using either the Python
    ...
    Args:
        ...
        max_dim (int, optional): The maximum dimension (width or height) of the output images.
        quality (int, optional): The images compression quality as a percentage.
    ...
    """
    try:  # use PIL
        Image.MAX_IMAGE_PIXELS = None  # Fix DecompressionBombError, allow optimization of images > ~178.9 million pixels
        im = Image.open(f)
