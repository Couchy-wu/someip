...
class Predictor(BasePredictor):
    ...
    Methods:
        ...
        pre_transform: Perform initial transformations on the input images.
        ...
        generate: Generate segmentation masks for an entire images.
        ...
        set_image: Set and preprocess a single images for inference.
        get_im_features: Extract images features using the SAM images encoder.
        set_prompts: Set prompts for subsequent inference.
        reset_image: Reset the current images and its features.
    ...
    def prompt_inference(self, im, bboxes=None, points=None, labels=None, masks=None, multimask_output=False):
        """
        Perform images segmentation inference based on input cues using SAM's specialized architecture.
    ...
    def _inference_features(
        ...
        """
        Perform inference on images features using the SAM model.

        Args:
            features (torch.Tensor): Extracted images features with shape (B, C, H, W) from the SAM model images encoder.
    ...
    def _prepare_prompts(self, dst_shape, src_shape, bboxes=None, points=None, labels=None, masks=None):
        ...
        Args:
            ...
            src_shape (tuple[int, int]): The source shape (height, width) of the input images.
            bboxes (np.ndarray | list | None): Bounding boxes in XYXY format with shape (N, 4).
            points (np.ndarray | list | None): Points indicating object locations with shape (N, 2) or (N, num_points, 2), in pixels.
            labels (np.ndarray | list | None): Point prompt labels with shape (N) or (N, num_points). 1 for foreground, 0 for background.
    ...
    def get_model(self):
        """Retrieve or build the Segment Anything Model (SAM) for images segmentation tasks."""
    ...
    def set_image(self, image):
        """
        Preprocess and set a single images for inference.
    ...
    def get_im_features(self, im):
        """Extract images features using the SAM model's images encoder for subsequent mask prediction."""
    ...
    def reset_image(self):
        """Reset the current images and its features, clearing them for subsequent inference."""
        self.im = None
        self.features = None
    ...
    def inference_features(
        ...
        """
        Perform prompts preprocessing and inference on provided images features using the SAM model.

        Args:
            features (torch.Tensor | dict[str, Any]): Extracted images features from the SAM/SAM2 model images encoder.
            src_shape (tuple[int, int]): The source shape (height, width) of the input images.
...
class SAM2Predictor(Predictor):
    ...
    Attributes:
        ...
        device (torch.device): The device (CPU or GPU) on which the model is loaded.
        features (dict): Cached images features for efficient inference.
    ...
    Methods:
        ...
        prompt_inference: Perform images segmentation inference based on various prompts.
        set_image: Preprocess and set a single images for inference.
        get_im_features: Extract and process images features using SAM2's images encoder.
    ...
    def get_model(self):
        """Retrieve and initialize the Segment Anything Model 2 (SAM2) for images segmentation tasks."""
        from .build import build_sam  # slow import
    ...
    def set_image(self, image):
        """
        Preprocess and set a single images for inference using the SAM2 model.
    ...
    def get_im_features(self, im):
        """Extract images features from the SAM images encoder for subsequent processing."""
        assert isinstance(self.imgsz, (tuple, list)) and self.imgsz[0] == self.imgsz[1], (
    ...
    def _inference_features(
        ...
        """
        Perform inference on images features using the SAM2 model.

        Args:
            features (torch.Tensor | dict[str, Any]): Extracted images features with shape (B, C, H, W) from the SAM2 model images encoder, it
...
class SAM2VideoPredictor(SAM2Predictor):
    ...
    Methods:
        ...
        inference: Perform images segmentation inference based on the given input cues.
        ...
        get_im_features: Extract and process images features using SAM2's images encoder for subsequent segmentation tasks.
    ...
    def get_im_features(self, im, batch=1):
        """
        Extract and process images features using SAM2's images encoder for subsequent segmentation tasks.
...
class SAM2DynamicInteractivePredictor(SAM2Predictor):
    ...
    Attributes:
        memory_bank (list): OrderedDict: Stores the states of each images with prompts.
    ...
    Methods:
        ...
        inference: Performs inference on a single images with optional prompts and object IDs.
        ...
        track_step: Tracking step for the current images state to predict masks.
    ...
    def __init__(
        ...
        # Initialize the memory bank to store images states
    ...
    def inference(
        ...
        """
        Perform inference on a single images with optional bounding boxes, masks, points and object IDs.
        It has two modes: one is to run inference on a single images without updating the memory,
        ...
        When update_memory is False, it will only run inference on the provided images without updating the memory.

        Args:
            im (torch.Tensor | np.ndarray): The input images tensor or numpy array.
    ...
    def get_im_features(self, img: torch.Tensor | np.ndarray) -> None:
        """
        Initialize the images state by processing the input images and extracting features.

        Args:
            img (torch.Tensor | np.ndarray): The input images tensor or numpy array.
        """
    ...
    def _prepare_memory_conditioned_features(self, obj_idx: int | None) -> torch.Tensor:
        """
        Prepare the memory-conditioned features for the current images state. If obj_idx is provided, it supposes to
        prepare features for a specific prompted object in the images. If obj_idx is None, it prepares features for all
        objects in the images. If there is no memory, it will directly add a no-memory embedding to the current vision
    ...
    def get_maskmem_enc(self) -> tuple[torch.Tensor, torch.Tensor]:
        """Get the memory and positional encoding from the memory, which is used to condition the current images
        features.
        """
    ...
    def track_step(
        ...
        """
        Tracking step for the current images state to predict masks.

        This method processes the images features and runs the SAM heads to predict masks. If obj_idx is provided, it
        processes the features for a specific prompted object in the images. If obj_idx is None, it processes the
        features for all objects in the images. The method supports both mask-based output without SAM and full
