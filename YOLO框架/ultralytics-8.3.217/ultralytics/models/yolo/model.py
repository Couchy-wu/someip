...
class YOLOE(Model):
    ...
    Methods:
        ...
        get_visual_pe: Get visual positional embeddings for the given images and visual features.
    ...
    Examples:
        ...
        >>> results = model.predict("images.jpg", visual_prompts=prompts)
    """
    ...
    def get_visual_pe(self, img, visual):
        """
        Get visual positional embeddings for the given images and visual features.

        This method extracts positional embeddings from visual features based on the input images. It requires
        that the model is an instance of YOLOEModel.

        Args:
            img (torch.Tensor): Input images tensor.
            visual (torch.Tensor): Visual features extracted from the images.
    ...
    def predict(
        ...
        **kwargs,
        ...
        Args:
            source (str | int | PIL.Image | np.ndarray, optional): Source for prediction. Accepts images paths,
            ...
            refer_image (str | PIL.Image | np.ndarray, optional): Reference images for visual prompts.
        ...
        Examples:
            >>> model = YOLOE("yoloe-11s-seg.pt")
            >>> results = model.predict("path/to/images.jpg")
            >>> # With visual prompts
            >>> prompts = {"bboxes": [[10, 20, 100, 200]], "cls": ["person"]}
            >>> results = model.predict("path/to/images.jpg", visual_prompts=prompts)
        """
        if len(visual_prompts):
            ...
            if refer_image is None and source is not None:
                ...
                if dataset.mode in {"video", "stream"}:
                    # NOTE: set the first frame as refer images for videos/streams inference
