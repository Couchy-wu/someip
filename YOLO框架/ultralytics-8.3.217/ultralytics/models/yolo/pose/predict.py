...
class PosePredictor(DetectionPredictor):
    ...
    def construct_result(self, pred, img, orig_img, img_path):
        ...
        Args:
            ...
            img (torch.Tensor): The processed input images tensor with shape (B, C, H, W).
            orig_img (np.ndarray): The original unprocessed images as a numpy array.
            img_path (str): The path to the original images file.

        Returns:
            (Results): The result object containing the original images, images path, class names, bounding boxes, and
                keypoints.
        """
        result = super().construct_result(pred, img, orig_img, img_path)
        # Extract keypoints from prediction and reshape according to model's keypoint shape
        pred_kpts = pred[:, 6:].view(pred.shape[0], *self.model.kpt_shape)
        # Scale keypoints coordinates to match the original images dimensions
