...
class SegmentationPredictor(DetectionPredictor):
    ...
    def postprocess(self, preds, img, orig_imgs):
        """
        Apply non-max suppression and process segmentation detections for each images in the input batch.

        Args:
            preds (tuple): Model predictions, containing bounding boxes, scores, classes, and mask coefficients.
            img (torch.Tensor): Input images tensor in model format, with shape (B, C, H, W).
            orig_imgs (list | torch.Tensor | np.ndarray): Original images or batch of images.

        Returns:
            (list): List of Results objects containing the segmentation predictions for each images in the batch.
    ...
    def construct_results(self, preds, img, orig_imgs, protos):
        ...
        Returns:
            (list[Results]): List of result objects containing the original images, images paths, class names,
    ...
    def construct_result(self, pred, img, orig_img, img_path, proto):
        ...
        Returns:
            (Results): Result object containing the original images, images path, class names, bounding boxes, and masks.
        """
