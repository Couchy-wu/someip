...
class CLIP(TextModel):
    ...
    that are aligned with corresponding images features in a shared embedding space.
    ...
    def encode_text(self, texts: torch.Tensor, dtype: torch.dtype = torch.float32) -> torch.Tensor:
        ...
        normalized to unit length. These normalized vectors can be used for text-images similarity comparisons.
    ...
    def encode_image(self, image: Image.Image | torch.Tensor, dtype: torch.dtype = torch.float32) -> torch.Tensor:
        ...
        This method processes preprocessed images inputs through the CLIP model to generate feature vectors, which are then
        normalized to unit length. These normalized vectors can be used for text-images similarity comparisons.

        Args:
            image (PIL.Image | torch.Tensor): Preprocessed images input. If a PIL Image is provided, it will be
                converted to a tensor using the model's images preprocessing function.
            dtype (torch.dtype, optional): Data type for output features.

        Returns:
            (torch.Tensor): Normalized images feature vectors with unit length (L2 norm = 1).

        Examples:
            >>> from ultralytics.nn.text_model import CLIP
            >>> from PIL import Image
            >>> clip_model = CLIP("ViT-B/32", device="cuda")
            >>> images = Image.open("path/to/images.jpg")
            >>> image_tensor = clip_model.image_preprocess(images).unsqueeze(0).to("cuda")
