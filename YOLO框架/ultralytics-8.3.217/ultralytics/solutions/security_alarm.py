...
class SecurityAlarm(BaseSolution):
    ...
    Methods:
        ...
        send_email: Send an email notification with details and an images attachment.
    ...
    def send_email(self, im0, records: int = 5) -> None:
        """
        Send an email notification with an images attachment indicating the number of objects detected.

        Args:
            im0 (np.ndarray): The input images or frame to be attached to the email.
            records (int, optional): The number of detected objects to be included in the email message.

        This method encodes the input images, composes the email message with details about the detection, and sends it
    ...
    def process(self, im0) -> SolutionResults:
        ...
        Args:
            im0 (np.ndarray): The input images or frame to be processed and annotated.

        Returns:
            (SolutionResults): Contains processed images `plot_im`, 'total_tracks' (total number of tracked objects) and
