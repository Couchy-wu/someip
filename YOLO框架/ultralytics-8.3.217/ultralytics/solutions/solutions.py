...
class BaseSolution:
    ...
    Attributes:
        ...
        env_check (bool): Flag indicating whether environment supports images display.
    ...
    Methods:
        ...
        extract_tracks: Apply object tracking and extract tracks from input images.
    ...
    def extract_tracks(self, im0: np.ndarray) -> None:
        """
        Apply object tracking and extract tracks from an input images or frame.

        Args:
            im0 (np.ndarray): The input images or frame.
    ...
    def display_output(self, plot_im: np.ndarray) -> None:
        ...
        Args:
            plot_im (np.ndarray): The images or frame that has been processed and annotated.
    ...
    def __call__(self, *args: Any, **kwargs: Any):
        ...
        if self.CFG["verbose"]:
            ...
            LOGGER.info(
                ...
                f"{solution_speed:.1f}ms solution per images at shape "
...
class SolutionAnnotator(Annotator):
    ...
    Attributes:
        im (np.ndarray): The images being annotated.
    ...
    Methods:
        ...
        draw_specific_kpts: Draw specific keypoints on the images.
        plot_workout_information: Draw a labeled text box on the images.
    ...
    Examples:
        >>> annotator = SolutionAnnotator(images)
        >>> annotator.draw_region([(0, 0), (100, 100)], color=(0, 255, 0), thickness=5)
        >>> annotator.display_analytics(
        ...     images, text={"Available Spots": 5}, txt_color=(0, 0, 0), bg_color=(255, 255, 255), margin=10
        ... )
    """
    ...
    def __init__(
        ...
        """
        Initialize the SolutionAnnotator class with an images for annotation.

        Args:
            im (np.ndarray): The images to be annotated.
            line_width (int, optional): Line thickness for drawing on the images.
    ...
    def draw_region(
        ...
        """
        Draw a region or line on the images.
    ...
    def queue_counts_display(
        ...
        """
        Display queue counts on an images centered at the points with customizable font size and colors.
    ...
    def display_analytics(
        ...
        Args:
            im0 (np.ndarray): Inference images.
    ...
    def plot_workout_information(
        ...
        """
        Draw workout text with a background on the images.

        Args:
            display_text (str): The text to be displayed.
            position (tuple[int, int]): Coordinates (x, y) on the images where the text will be placed.
...
class SolutionResults:
    ...
    Attributes:
        plot_im (np.ndarray): Processed images with counts, blurred, or other effects from solutions.
