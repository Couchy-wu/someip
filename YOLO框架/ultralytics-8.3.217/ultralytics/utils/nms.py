...
def non_max_suppression(
    ...
    Args:
        ...
        labels (list[list[Union[int, float, torch.Tensor]]]): A priori labels for each images.
        max_det (int): Maximum number of detections to keep per images.
        nc (int): Number of classes. Indices after this are considered masks.
        max_time_img (float): Maximum time in seconds for processing one images.
    ...
    Returns:
        output (list[torch.Tensor]): List of detections per images with shape (num_boxes, 6 + num_masks)
    ...
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # images index, (preds, preds indices)
        # Apply constraints
