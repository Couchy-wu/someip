# -*- coding: utf-8 -*-
"""can_core.transmit —— 报文发送与周期发送

从 device.py 拆出，聚焦"把报文发出去"：
  · 单帧发送（CAN / CANFD）、信号级发送（Send_Can_Signal）
  · 自动（周期）发送的建立、清除与按索引移除、动态间隔发送
共享状态见 can_core.can_state。
"""
import logging
import threading
import time
...
def Send_Can(chn_handle, stdorext, id, data, round):
    ...
    for i in range(transmit_num):
        msgs[i].transmit_type = state.transmit_type   # 0-正常发送，1-单次发发送，2-自发自收，3-单次自发自收
    ...
    ret = state.zcanlib.Transmit(chn_handle, msgs, transmit_num)
...
def Send_Canfd(chn_handle, stdorext, id, data, round):
    ...
    for i in range(transmit_canfd_num):
        canfd_msgs[i].transmit_type = state.transmit_type     # 0-正常发送，2-自发自收
    ...
    ret = state.zcanlib.TransmitFD(chn_handle, canfd_msgs, transmit_canfd_num)
...
def Clear_Auto_Can_Send(device_handle, chn):
    """清除指定通道的定时发送列表"""
    ret = state.zcanlib.ZCAN_SetValue(device_handle, str(chn) + "/clear_auto_send", "0".encode("utf-8"))
...
def Enable_Auto_Can_Send(device_handle, chn):
    """使能指定通道的所有定时发送任务"""
    ret = state.zcanlib.ZCAN_SetValue(device_handle, str(chn) + "/apply_auto_send", "0".encode("utf-8"))
    if ret != ZCAN_STATUS_OK:
...
def Auto_Send_Can(device_handle, chn, stdorext, id, data, signal_cycle, index=0):
    ...
    auto_can.obj.transmit_type = state.transmit_type      # 0-正常发送，1-单次发发送，2-自发自收，3-单次自发自收
    ...
    ret = state.zcanlib.ZCAN_SetValue(device_handle,str(chn)+"/auto_send",byref(auto_can))
    if ret != ZCAN_STATUS_OK:
...
def Auto_Send_Canfd(device_handle, chn, stdorext, id, data, signal_cycle, index=0):
    ...
    auto_canfd.obj.transmit_type = state.transmit_type       # 0-正常发送，1-单次发发送，2-自发自收，3-单次自发自收
    ...
    # 将发送任务配置写入到导通的定时发送列表中
    ret = state.zcanlib.ZCAN_SetValue(device_handle, str(chn) + "/auto_send_canfd", byref(auto_canfd))
    if ret != ZCAN_STATUS_OK:
