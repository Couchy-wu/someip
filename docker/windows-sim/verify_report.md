...
**✅ 结果：19 通过 / 0 失败 / 0 跳过**（共 19 项，通过率 100.0%，耗时 42.82s）

- 生成时间: `2026-09-21 14:10:08`
...
- 上次运行: `2026-09-21 14:09:15`（判定 PASS）
- 无变化（与上次逐项一致）
...
| 4 | 4. 界面字体与中文渲染字体 | ✅ PASS | 0.1 | selected='微软雅黑', 可用字体=0, PIL字体=<_io.BytesIO object at 0x00000000017440E0> |
...
| 8 | 8. 全部界面与工具模块导入 | ✅ PASS | 0.2 | 72/72 模块全部导入成功 |
| 9 | 9. CAN 驱动库探测与加载（桩库） | ✅ PASS | 0.0 | 桩库加载成功并通过调用验证：Py_GetVersion()=473576736；本机库 zlgcan.dll(接口=unknown) |
...
| 16 | 16. 信号矩阵 XLSX → CSV 转换 | ✅ PASS | 0.3 | openpyxl 读写 OK，模块入口=XlsmToCsvConverter |
| 17 | 17. 项目自带自检脚本可运行 | ✅ PASS | 11.0 | check_imports.py rc=0 (项目内部导入全部可解析 ✓); check_static.py rc=0 (静态检查通过 ✓); selftest.py rc=0 (============================================================) |
| 18 | 18. 单元测试（pytest） | ✅ PASS | 21.2 | ......... [100%] |
| 19 | 19. SOME/IP 回放窗口（布局与降级） | ✅ PASS | 2.3 | 23 个事件，字段数 {'RTK': 27, 'PilotStatus': 7, 'VehiclePosition': 34, 'HudNavmap': 3}；状态=SOME/IP 库不可用（动作已置灰）；库不可用(已降级) |
