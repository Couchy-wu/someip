...
**✅ 结果：19 通过 / 0 失败 / 0 跳过**（共 19 项，通过率 100.0%，耗时 88.59s）

- 生成时间: `2026-09-21 15:33:58`
...
- 上次运行: `2026-09-21 15:23:25`（判定 PASS）
- 上次运行: `2026-09-21 15:23:25`（判定 PASS）
- 无变化（与上次逐项一致）
...
| 7 | 7. main.py 导入链（不启动 GUI 主循环） | ✅ PASS | 2.1 | import main 成功；MainWindow/TextRedirector 可用；导出=3 项 |
...
| 15 | 15. CAN 信号 → 数据字节（outputMatrix.csv） | ✅ PASS | 0.8 | 信号 0x095/Eng_Start_Result_Fdbk_Info_S（位=1.0-1.1, 长度=8）: 枚举0→[0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00] 枚举1→[0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00] 值随枚举变化=True |
| 16 | 16. 信号矩阵 XLSX → CSV 转换 | ✅ PASS | 0.4 | openpyxl 读写 OK，模块入口=XlsmToCsvConverter |
| 17 | 17. 项目自带自检脚本可运行 | ✅ PASS | 13.2 | check_imports.py rc=0 (项目内部导入全部可解析 ✓); check_static.py rc=0 (静态检查通过 ✓); selftest.py rc=0 (============================================================) |
| 18 | 18. 单元测试（pytest） | ✅ PASS | 64.1 | .ss....................................... [100%] |
| 19 | 19. SOME/IP 回放窗口（布局与降级） | ✅ PASS | 2.8 | 23 个事件，字段数 {'RTK': 27, 'PilotStatus': 7, 'VehiclePosition': 34, 'HudNavmap': 3}；状态=SOME/IP 库不可用（动作已置灰）｜原因：未找到 S；库不可用(已降级) |
