...
```
┌───────────────────────────────────────────────────────────────────────────────────┐
│ [打开服务][启动服务][停止服务][关闭服务] │ [重新检测库][导出服务表]        [保存配置] │
│ 状态：库就绪｜阶段：已启动｜未回放（已发 0）｜服务 11/事件 23                        │
├──────────────────────────────────┬────────────────────────────────────────────────┤
│ ① 网络与 SD 配置                  │ ③ pcap 回放控制                                 │
...
│   [全选][全不选][仅结构化类型] 过滤[]│   类型 [RTK ▾] [载入示例值][清零字段]             │
│  ┌──────────────────────────────┐ │   [发送该事件]                                  │
│  │注册 服务   事件   名称  类型  TP│ │   ┌────────────┬───────────┬──────────┐        │
│  │ ☑  0x000A 0x8001 Vehicle… 否 │ │   │ 字段        │ 值        │ 类型     │        │
│  │ ☑  0x000B 0x8001 RTKInfo… 否 │ │   │ Counter     │ 1         │ 整数     │        │
...
工具栏按用途分三组（组间用竖分隔线）：**服务生命周期**（打开/启动/停止/关闭）、
**库与导出**（重新检测库/导出服务表）、**配置保存**；按钮由 `ActionBar` 自动排布，
样式走 `Theme`（主操作蓝、危险操作红、次要灰）。

**状态栏独占工具栏第二行**：库不可用时的"红字 + 原因"实测约 820px，与按钮挤在同一行会被
窗口裁掉（1120px 宽），单独一行才能完整显示。「发送该事件」同理独占 ④ 的第二行 ——
右半区只有约 470px，与其他按钮挤一行会把主操作顶出可视区（实测 x=471 > 453）。
...
对应的单测在 `tests/test_someip_gui.py`：`test_rule_table_pure_transitions`（规则表逐状态）、
