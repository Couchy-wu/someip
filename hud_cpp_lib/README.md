# C++ 服务端库（libarhud_server.so）+ Python 调用

用 **C++ 编写 SOME/IP 服务端库**（内部 vsomeip 3.4.10），**Python 通过 ctypes 调用**：
Python 负责业务编排（指定 pcap 回放、对数据结构赋值组包），C++ 库负责通信内核
（offer 11 服务/23 事件、发送、pcap 回放 + SOME/IP-TP 重组）。
...
```bash
# 依赖：vsomeip 3.4.10 + zlib（见仓库 docker/Dockerfile）
cd hud_cpp_lib && make          # 生成 libarhud_server.so
```
...
| 51KB 大消息 0x000C:8003 | **37+ 条**（someip-tp 分片送达，0 丢弃） |
| 客户端总接收 | 1700+ 条（14 个事件） |
