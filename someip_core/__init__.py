...
模块划分：
    models.py    服务/事件定义表（11 服务 / 23 事件）与数据类型
    api.py       ctypes 绑定（惰性加载；结构体、序列化、回放、发送）
    pcap_info.py pcap 解析与摘要（TP 分片重组；供界面展示与预检）
    config.py    回放配置读写（data/someip/replay_config.json）
    replay.py    回放控制器（会话/注册/启动/回放/单条发送/状态）

依赖约束：可依赖 hudcore（平台与路径）与标准库；不依赖任何界面层。
"""
from .config import ReplayConfig, config_path, export_service_table, table_path
from .models import (
