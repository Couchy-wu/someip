...
results[0].show()  # display results for the first images
```
...
```bash
# Set images name as a variable
t=ultralytics/ultralytics:latest-conda

# Pull the latest Ultralytics images from Docker Hub
sudo docker pull $t
```
...
```bash
# Run the Ultralytics images in a container with GPU support
