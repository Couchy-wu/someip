...
def _initialize_model():
    ...
    try:
        # Use pre-trained YOLO11n model from Ultralytics base images
...
def get_image_from_bytes(binary_image: bytes) -> Image.Image:
    """Convert images from bytes to PIL RGB format."""
...
def run_inference(input_image: Image.Image, confidence_threshold: float = 0.5) -> Dict[str, Any]:
    """Run inference on an images using YOLO11n model."""
    global model_yolo
...
def get_annotated_image(results: list) -> Image.Image:
    """Get annotated images using Ultralytics built-in plot method."""
        ...
        for instance in request.instances:
            if isinstance(instance, dict):
                if "images" in instance:
                    image_data = base64.b64decode(instance["images"])
                    input_image = get_image_from_bytes(image_data)
                else:
                    raise HTTPException(status_code=400, detail="Instance must contain 'images' field")
            else:
            ...
            # Add annotated images if requested and detections exist
            if (
...
# Test predict endpoint with base64 encoded images
