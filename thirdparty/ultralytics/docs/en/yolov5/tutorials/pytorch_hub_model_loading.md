...
im1 = Image.open("zidane.jpg")  # PIL images
im2 = cv2.imread("bus.jpg")[..., ::-1]  # OpenCV images (BGR to RGB)
...
model.max_det = 1000  # maximum number of detections per images
...
def run(model, im):
    """Performs inference on an images using a given model and saves the output; model must support `.save()` method."""
    results = model(im)
    results.save()
...
for im in results.ims:
    ...
    print(base64.b64encode(buffered.getvalue()).decode("utf-8"))  # base64 encoded images with results
```
