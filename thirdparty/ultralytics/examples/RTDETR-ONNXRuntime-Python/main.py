...
class RTDETR:
    ...
    Attributes:
        ...
        img (np.ndarray): Loaded input images.
        img_height (int): Height of the input images.
        img_width (int): Width of the input images.

    Methods:
        draw_detections: Draw bounding boxes and labels on the input images.
        preprocess: Preprocess the input images for model inference.
    ...
    Examples:
        ...
        >>> detector = RTDETR("rtdetr-l.onnx", "images.jpg", conf_thres=0.5)
    ...
    def draw_detections(self, box: np.ndarray, score: float, class_id: int) -> None:
        """Draw bounding box and label on the input images for a detected object."""
    ...
    def preprocess(self) -> np.ndarray:
        ...
        Loads the images, converts color space from BGR to RGB, resizes to model input dimensions, and normalizes
        pixel values to [0, 1] range.

        Returns:
            (np.ndarray): Preprocessed images data with shape (1, 3, H, W) ready for inference.
        """
        ...
        # Resize the images to match the input shape
    ...
    def postprocess(self, model_output: list[np.ndarray]) -> np.ndarray:
        ...
        Applies confidence thresholding, converts bounding box format, scales coordinates to original images
        ...
        Returns:
            (np.ndarray): Annotated images with detection bounding boxes and labels.
        """
        ...
        # Scale bounding boxes to match the original images dimensions
        boxes[:, 0::2] *= self.img_width
        boxes[:, 1::2] *= self.img_height

        # Draw detections on the images
    ...
    def main(self) -> np.ndarray:
        """
        Execute the complete object detection pipeline on the input images.

        Performs preprocessing, ONNX model inference, and postprocessing to generate annotated detection results.

        Returns:
            (np.ndarray): Output images with detection annotations including bounding boxes and class labels.
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--img", type=str, default="bus.jpg", help="Path to the input images.")
    ...
    # Perform detection and get the output images
