...
max_det = 20  # Maximum objects per images (increase for crowded scenes)
...
def extend_line_from_edge(mid_x: int, mid_y: int, direction: str, img_shape: tuple[int, int, int]) -> tuple[int, int]:
    """
    Calculate the endpoint to extend a line from the center toward an images edge.
...
def draw_tracking_scope(im, bbox: tuple, color: tuple) -> None:
    """
    Draw tracking scope lines extending from the bounding box to images edges.
