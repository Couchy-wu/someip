...
class YOLOv8Seg:
    ...
    Attributes:
        ...
        imgsz (tuple[int, int]): Input images size as (height, width) for the model.
    ...
    Methods:
        letterbox: Resize and pad images while maintaining aspect ratio.
        preprocess: Preprocess the input images before feeding it into the model.
    ...
    def __init__(self, onnx_model: str, conf: float = 0.25, iou: float = 0.7, imgsz: int | tuple[int, int] = 640):
        ...
        Args:
            ...
            imgsz (int | tuple[int, int], optional): Input images size of the model. Can be an integer for square
    ...
    def __call__(self, img: np.ndarray) -> list[Results]:
        """
        Run inference on the input images using the ONNX model.

        Args:
            img (np.ndarray): The original input images in BGR format.
    ...
    def letterbox(self, img: np.ndarray, new_shape: tuple[int, int] = (640, 640)) -> np.ndarray:
        ...
        Args:
            img (np.ndarray): Input images in BGR format.
            new_shape (tuple[int, int], optional): Target shape as (height, width).

        Returns:
            (np.ndarray): Resized and padded images.
        """
    ...
    def preprocess(self, img: np.ndarray, new_shape: tuple[int, int]) -> np.ndarray:
        """
        Preprocess the input images before feeding it into the model.

        Args:
            img (np.ndarray): The input images in BGR format.
            new_shape (tuple[int, int]): The target shape for resizing as (height, width).

        Returns:
            (np.ndarray): Preprocessed images ready for model inference, with shape (1, 3, height, width) and
                normalized to [0, 1].
        """
    ...
    def postprocess(self, img: np.ndarray, prep_img: np.ndarray, outs: list) -> list[Results]:
        ...
        Args:
            img (np.ndarray): The original input images.
            prep_img (np.ndarray): The preprocessed images used for inference.
    ...
    def process_mask(
        ...
        Args:
            ...
            shape (tuple[int, int]): The size of the input images as (height, width).
        ...
        masks = ops.scale_masks(masks[None], shape)[0]  # Scale masks to original images size
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source", type=str, default=str(ASSETS / "bus.jpg"), help="Path to input images")
