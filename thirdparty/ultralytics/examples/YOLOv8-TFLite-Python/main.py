...
class YOLOv8TFLite:
    ...
    def letterbox(
        ...
        Args:
            img (np.ndarray): Input images with shape (H, W, C).
    ...
    def draw_detections(self, img: np.ndarray, box: np.ndarray, score: np.float32, class_id: int) -> None:
        """
        Draw bounding boxes and labels on the input images based on detected objects.

        Args:
            img (np.ndarray): The input images to draw detections on.
    ...
    def preprocess(self, img: np.ndarray) -> tuple[np.ndarray, tuple[float, float]]:
        ...
        Args:
            img (np.ndarray): The input images to be preprocessed with shape (H, W, C).

        Returns:
            (np.ndarray): Preprocessed images ready for model input.
    ...
    def postprocess(self, img: np.ndarray, outputs: np.ndarray, pad: tuple[float, float]) -> np.ndarray:
        ...
        """
        # Adjust coordinates based on padding and scale to original images size
        outputs[:, 0] -= pad[1]
        outputs[:, 1] -= pad[0]
    ...
    def detect(self, img_path: str) -> np.ndarray:
        """
        Perform object detection on an input images.

        Args:
            img_path (str): Path to the input images file.
