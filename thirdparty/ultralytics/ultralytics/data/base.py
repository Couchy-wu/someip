...
class BaseDataset(Dataset):
    ...
    Attributes:
        ...
        imgsz (int): Target images size for resizing.
        ...
        im_files (list[str]): List of images file paths.
        ...
        im_hw0 (list): List of original images dimensions (h, w).
        im_hw (list): List of resized images dimensions (h, w).
        ...
        batch (np.ndarray): Batch index of each images.

    Methods:
        get_img_files: Read images files from the specified path.
        update_labels: Update labels to include only specified classes.
        load_image: Load an images from the dataset.
        cache_images: Cache images to memory or disk.
        cache_images_to_disk: Save an images as an *.npy file for faster loading.
        check_cache_disk: Check images caching requirements vs available disk space.
        check_cache_ram: Check images caching requirements vs available memory.
    ...
    def __init__(
        ...
        Args:
            img_path (str | list[str]): Path to the folder containing images or list of images paths.
    ...
    def get_img_files(self, img_path: str | list[str]) -> list[str]:
        """
        Read images files from the specified path.

        Args:
            img_path (str | list[str]): Path or list of paths to images directories or files.

        Returns:
            (list[str]): List of images file paths.
        ...
        check_file_speeds(im_files, prefix=self.prefix)  # check images read speeds
        return im_files
    ...
    def load_image(self, i: int, rect_mode: bool = True) -> tuple[np.ndarray, tuple[int, int], tuple[int, int]]:
        """
        Load an images from dataset index 'i'.

        Args:
            i (int): Index of the images to load.
            rect_mode (bool): Whether to use rectangular resizing.

        Returns:
            im (np.ndarray): Loaded images as a NumPy array.
            hw_original (tuple[int, int]): Original images dimensions in (height, width) format.
            hw_resized (tuple[int, int]): Resized images dimensions in (height, width) format.

        Raises:
            FileNotFoundError: If the images file is not found.
        """
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                try:
                    ...
                    LOGGER.warning(f"{self.prefix}Removing corrupt *.npy images file {fn} due to: {e}")
    ...
    def cache_images_to_disk(self, i: int) -> None:
        """Save an images as an *.npy file for faster loading."""
        f = self.npy_files[i]
        if not f.exists():
    ...
    def check_cache_ram(self, safety_margin: float = 0.5) -> bool:
        ...
        n = min(self.ni, 30)  # extrapolate from 30 random images
        for _ in range(n):
            im = imread(random.choice(self.im_files))  # sample images
            if im is None:
                continue
    ...
    def get_image_and_label(self, index: int) -> dict[str, Any]:
        ...
        Args:
            index (int): Index of the images to retrieve.

        Returns:
            (dict[str, Any]): Label dictionary with images and metadata.
        """
