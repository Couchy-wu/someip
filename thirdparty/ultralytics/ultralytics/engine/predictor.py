...
class BasePredictor:
    ...
    Attributes:
        ...
        plotted_img (np.ndarray): Last plotted images.
    ...
    Methods:
        preprocess: Prepare input images before inference.
        inference: Run inference on a given images.
    ...
    def preprocess(self, im: torch.Tensor | list[np.ndarray]) -> torch.Tensor:
        """
        Prepare input images before inference.

        Args:
            im (torch.Tensor | list[np.ndarray]): Images of shape (N, 3, H, W) for tensor, [(H, W, 3) x N] for list.

        Returns:
            (torch.Tensor): Preprocessed images tensor of shape (N, 3, H, W).
        """
    ...
    def inference(self, im: torch.Tensor, *args, **kwargs):
        """Run inference on a given images using the specified model and arguments."""
        visualize = (
    ...
    def postprocess(self, preds, img, orig_imgs):
        """Post-process predictions for an images and return them."""
        return preds

    def __call__(self, source=None, model=None, stream: bool = False, *args, **kwargs):
        """
        Perform inference on an images or stream.
    ...
    def write_results(self, i: int, p: Path, im: torch.Tensor, s: list[str]) -> str:
        ...
        Args:
            i (int): Index of the current images in the batch.
            p (Path): Path to the current images.
            im (torch.Tensor): Preprocessed images tensor.
    ...
    def show(self, p: str = ""):
        ...
        cv2.imshow(p, im)
        if cv2.waitKey(300 if self.dataset.mode == "images" else 1) & 0xFF == ord("q"):  # 300ms if images; else 1ms
            raise StopIteration
