...
def is_box_near_crop_edge(
    ...
    """
    Determine if bounding boxes are near the edge of a cropped images region using a specified tolerance.

    Args:
        boxes (torch.Tensor): Bounding boxes in XYXY format.
        crop_box (list[int]): Crop box coordinates in [x0, y0, x1, y1] format.
        orig_box (list[int]): Original images box coordinates in [x0, y0, x1, y1] format.
...
def generate_crop_boxes(
    ...
    """
    Generate crop boxes of varying sizes for multiscale images processing, with layered overlapping regions.

    Args:
        im_size (tuple[int, ...]): Height and width of the input images.
