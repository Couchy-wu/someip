...
class YOLOEVPDetectPredictor(DetectionPredictor):
    ...
    def pre_transform(self, im):
        ...
        This method applies letterboxing to the input images and transforms the visual prompts
        (bounding boxes or masks) accordingly.

        Args:
            im (list): List containing a single input images.
    ...
    def _process_single_image(self, dst_shape, src_shape, category, bboxes=None, masks=None):
        """
        Process a single images by resizing bounding boxes or masks and generating visuals.

        Args:
            dst_shape (tuple): The target shape (height, width) of the images.
            src_shape (tuple): The original shape (height, width) of the images.
            category (str): The category of the images for visual prompts.
            bboxes (list | np.ndarray, optional): A list of bounding boxes in the format [x1, y1, x2, y2].
            masks (np.ndarray, optional): A list of masks corresponding to the images.

        Returns:
            (torch.Tensor): The processed visuals for the images.
    ...
    def inference(self, im, *args, **kwargs):
        ...
        Args:
            im (torch.Tensor): Input images tensor.
    ...
    def get_vpe(self, source):
        ...
        Args:
            source (str | Path | int | PIL.Image | np.ndarray | torch.Tensor | list | tuple): The source
                of the images to make predictions on. Accepts various types including file paths, URLs, PIL
        ...
        assert len(self.dataset) == 1, "get_vpe only supports one images!"
