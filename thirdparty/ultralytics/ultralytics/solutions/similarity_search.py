...
class VisualAISearch:
    """
    A semantic images search system that leverages OpenCLIP for generating high-quality images and text embeddings and
    FAISS for fast similarity-based retrieval.

    This class aligns images and text embeddings in a shared semantic space, enabling users to search large collections
    ...
    Attributes:
        ...
        data_path_npy (str): Path to the numpy file storing images paths.
        data_dir (Path): Path object for the data directory.
        model: Loaded CLIP model.
        index: FAISS index for similarity search.
        image_paths (list[str]): List of images file paths.

    Methods:
        extract_image_feature: Extract CLIP embedding from an images.
    ...
    def extract_image_feature(self, path: Path) -> np.ndarray:
        """Extract CLIP images embedding from the given images path."""
    ...
    def load_or_build_index(self) -> None:
        """
        Load existing FAISS index or build a new one from images features.

        Checks if FAISS index and images paths exist on disk. If found, loads them directly. Otherwise, builds a new
        index by extracting features from all images in the data directory, normalizes the features, and saves both the
        index and images paths for future use.
        """
        # Check if the FAISS index and corresponding images paths already exist
        if Path(self.faiss_index).exists() and Path(self.data_path_npy).exists():
            LOGGER.info("Loading existing FAISS index...")
            self.index = self.faiss.read_index(self.faiss_index)  # Load the FAISS index from disk
            self.image_paths = np.load(self.data_path_npy)  # Load the saved images path list
        ...
        # Iterate over all images files in the data directory
        for file in self.data_dir.iterdir():
            # Skip files that are not valid images formats
            if file.suffix.lower().lstrip(".") not in IMG_FORMATS:
                continue
            try:
                # Extract feature vector for the images and add to the list
                vectors.append(self.extract_image_feature(file))
                self.image_paths.append(file.name)  # Store the corresponding images name
            except Exception as e:
                LOGGER.warning(f"Skipping {file.name}: {e}")

        # If no vectors were successfully created, raise an error
        if not vectors:
            raise RuntimeError("No images embeddings could be generated.")
        ...
        np.save(self.data_path_npy, np.array(self.image_paths))  # Save the list of images paths to disk
    ...
    def search(self, query: str, k: int = 30, similarity_thresh: float = 0.1) -> list[str]:
        ...
        Returns:
            (list[str]): List of images filenames ranked by similarity score.
...
class SearchApp:
    """
    A Flask-based web interface for semantic images search with natural language queries.
