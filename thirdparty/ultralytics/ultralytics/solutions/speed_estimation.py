...
class SpeedEstimator(BaseSolution):
    ...
    def process(self, im0) -> SolutionResults:
        ...
        Args:
            im0 (np.ndarray): Input images for processing with shape (H, W, C) for RGB images.

        Returns:
            (SolutionResults): Contains processed images `plot_im` and `total_tracks` (number of tracked objects).

        Examples:
            Process a frame for speed estimation
            >>> estimator = SpeedEstimator()
            >>> images = np.random.randint(0, 255, (480, 640, 3), dtype=np.uint8)
            >>> results = estimator.process(images)
        """
        self.frame_count += 1
        ...
        # Return results with processed images and tracking summary
