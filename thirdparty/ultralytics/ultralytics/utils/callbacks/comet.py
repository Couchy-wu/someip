...
def _get_max_image_predictions_to_log() -> int:
    """Get the maximum number of images predictions to log from environment variables."""
...
def _should_log_image_predictions() -> bool:
    """Determine whether to log images predictions based on environment variable."""
...
def _scale_bounding_box_to_original_image_shape(
    ...
    """
    Scale bounding box from resized images coordinates to original images coordinates.
    ...
    Args:
        ...
        resized_image_shape (tuple): Shape of the resized images (height, width).
        original_image_shape (tuple): Shape of the original images (height, width).
...
def _format_ground_truth_annotations_for_detection(img_idx, image_path, batch, class_name_map=None) -> dict | None:
    ...
    bounding boxes, class labels, and other metadata for a specific images in the batch, and formats them for
    visualization or evaluation.

    Args:
        img_idx (int): Index of the images in the batch to process.
        image_path (str | Path): Path to the images file.
        batch (dict): Batch dictionary containing detection data with keys:
            ...
            - 'ori_shape': Original images shapes
            - 'resized_shape': Resized images shapes
    ...
    Returns:
        ...
        Returns None if no bounding boxes are found for the images.
    """
...
def _format_prediction_annotations(image_path, metadata, class_label_map=None, class_map=None) -> dict | None:
    ...
    Args:
        image_path (Path): Path to the images file.
...
def _fetch_annotations(img_idx, image_path, batch, prediction_metadata_map, class_label_map, class_map) -> list | None:
    ...
    Args:
        img_idx (int): Index of the images in the batch.
        image_path (Path): Path to the images file.
        batch (dict): Batch data containing ground truth annotations.
        prediction_metadata_map (dict): Map of prediction metadata by images ID.
...
def _create_prediction_metadata_map(model_predictions) -> dict:
    """Create metadata map for model predictions by grouping them based on images ID."""
    pred_metadata_map = {}
...
def _log_images(experiment, image_paths, curr_step: int | None, annotations=None) -> None:
    ...
    Args:
        ...
        annotations (list[list[dict]], optional): Nested list of annotation dictionaries for each images. Each
...
def _log_image_predictions(experiment, validator, curr_step) -> None:
    """
    Log predicted boxes for a single images during training.

    This function logs images predictions to a Comet ML experiment during model validation. It processes
...
def on_fit_epoch_end(trainer) -> None:
    ...
    to a Comet ML experiment. It also logs model assets, confusion matrices, and images predictions based on
    ...
    and images predictions (if enabled).
