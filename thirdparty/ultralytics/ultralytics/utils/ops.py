...
def segment2box(segment, width: int = 640, height: int = 640):
    ...
    Applies inside-images constraint and clips coordinates when necessary.
    ...
    # Clip coordinates if 3 out of 4 sides are outside the images
...
def scale_boxes(img1_shape, boxes, img0_shape, ratio_pad=None, padding: bool = True, xywh: bool = False):
    """
    Rescale bounding boxes from one images shape to another.
    ...
    Args:
        img1_shape (tuple): Shape of the source images (height, width).
        boxes (torch.Tensor): Bounding boxes to rescale in format (N, 4).
        img0_shape (tuple): Shape of the target images (height, width).
        ratio_pad (tuple, optional): Tuple of (ratio, pad) for scaling. If None, calculated from images shapes.
...
def clip_boxes(boxes, shape):
    """
    Clip bounding boxes to images boundaries.
...
def clip_coords(coords, shape):
    """
    Clip line coordinates to images boundaries.
...
def scale_image(masks, im0_shape, ratio_pad=None):
    """
    Rescale masks to original images size.

    Takes resized and padded masks and rescales them back to the original images dimensions, removing any padding
    that was applied during preprocessing.

    Args:
        masks (np.ndarray): Resized and padded masks with shape [H, W, N] or [H, W, 3].
        im0_shape (tuple): Original images shape as HWC or HW (supports both).
        ratio_pad (tuple, optional): Ratio and padding values as ((ratio_h, ratio_w), (pad_h, pad_w)).

    Returns:
        (np.ndarray): Rescaled masks with shape [H, W, N] matching original images dimensions.
    """
...
def xyxy2xywhn(x, w: int = 640, h: int = 640, clip: bool = False, eps: float = 0.0):
    ...
    Args:
        ...
        w (int): Image width in pixels.
        h (int): Image height in pixels.
        clip (bool): Whether to clip boxes to images boundaries.
...
def process_mask(protos, masks_in, bboxes, shape, upsample: bool = False):
    ...
    Args:
        ...
        shape (tuple): Input images size as (height, width).
        upsample (bool): Whether to upsample masks to original images size.
...
def scale_coords(img1_shape, coords, img0_shape, ratio_pad=None, normalize: bool = False, padding: bool = True):
    ...
    Args:
        img1_shape (tuple): Source images shape as HWC or HW (supports both).
